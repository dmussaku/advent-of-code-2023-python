...
def parse_file(file_path) -> Dict[str, List[Union[List[int], int]]]:
    ...
    for line in filter(lambda x: x not in ("", "\n"), lines):
        if line.startswith("seeds:"):
            ...
            result["seeds"] = [
                int(val)
                for val in filter(lambda x: x != "", rest.strip("\n").split(" "))
            ]
        elif ":" in line:
            ...
            result[current_key].append(
                [
                    int(val)
                    for val in filter(lambda x: x != "", line.strip("\n").split(" "))
                ]
            )
    return result
...
def source_destination_range_map(
    number_range: range, destination_source_ranges: List[List[int]]
) -> List[range]:
    resulting_ranges = []
...
def run_part_1(input):
    ...
    for seed in seeds:
        ...
        for key, destination_source_ranges in input_map.items():
            current_position = source_destination_map(
                current_position, destination_source_ranges
            )
